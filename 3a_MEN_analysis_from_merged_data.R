...
# REQUIRES a saved csv file generated by 2a_MEN_localization_from_merged_data.R
...
# Mito MEAN localization measure -- Statistics -----
...
# Mito INTDEN measure -- Statistics


foldchange_25filt <- mean(o25_restrfilt$FractionInMito)/mean(o25_permfilt$FractionInMito)
foldchange_25 <- mean(o25_restr$FractionInMito)/mean(o25_perm$FractionInMito)

wilcoxon_25filt <- wilcox.test(FractionInMito ~ Temp, data = corrected_mito_loc_xc_25filt)$p.value
wilcoxon_25 <- wilcox.test(FractionInMito ~ Temp, data = corrected_mito_loc_xc_25)$p.value

foldchange_35filt <- mean(o35_restrfilt$FractionInMito)/mean(o35_permfilt$FractionInMito)
foldchange_35 <- mean(o35_restr$FractionInMito)/mean(o35_perm$FractionInMito)

wilcoxon_35filt <- wilcox.test(FractionInMito ~ Temp, data = corrected_mito_loc_xc_35filt)$p.value
wilcoxon_35 <- wilcox.test(FractionInMito ~ Temp, data = corrected_mito_loc_xc_35)$p.value

foldchange_50filt <- mean(o50_restrfilt$FractionInMito)/mean(o50_permfilt$FractionInMito)
foldchange_50 <- mean(o50_restr$FractionInMito)/mean(o50_perm$FractionInMito)

wilcoxon_50filt <- wilcox.test(FractionInMito ~ Temp, data = corrected_mito_loc_xc_50filt)$p.value
wilcoxon_50 <- wilcox.test(FractionInMito ~ Temp, data = corrected_mito_loc_xc_50)$p.value
...
plot25mean <- ggplot(corrected_mito_loc_xc_25, aes(factor(Temp), MitoLoc)) +
...
plot25intden <- ggplot(corrected_mito_loc_xc_25, aes(factor(Temp), FractionInMito)) +
  geom_boxplot(color = "black", fill = "blue", alpha = 0.5) +
  scale_y_continuous(limits=c(0,8)) +
  labs(title =  paste("Fractional Localization,", strain),
       x = "Temperature", y = "Mito IntDen / Cell IntDen") +
  theme_bw() +
  theme(text = element_text(size=18), plot.title = element_text(size = 14, face="bold"))
...
# 
ggsave(plot = plot25mean, filename = file.path(parentDir, paste(Sys.Date(), strain, "Mean offset -25.png")))
ggsave(plot = plot25intden, filename = file.path(parentDir, paste(Sys.Date(), strain, "IntDen offset -25.png")))
