...
# -- calculates mito localization (mean) as ratio of mito mean to whole-cell mean (whole-cell mean = mean of "cookie cutter" ROI drawn around cell)
# -- TODO: calculate mito localization (intden) as ratio of mito intden to whole-cell intden ( = intden of "cookie cutter" ROI) 
...
# Mean of whole-cell ROI is [under]estimate of true cytoplasmic mean since it is a cookie-cutter and includes some extracellular background
...
cell_intden_corr <- bkgcorr(cell_mito_xc_bkd$RawCellMean,
                     cell_mito_xc_bkd$CellVolume_um3,
                     voxel_size,
                     cell_mito_xc_bkd$ExtracellularBackground)

mito_intden_corr <- bkgcorr(cell_mito_xc_bkd$RawMitoMean,
                     cell_mito_xc_bkd$MitoVolume_um3,
                     voxel_size,
                     cell_mito_xc_bkd$ExtracellularBackground)


# Fraction of green intden in mito ----
# note this is a "cookie-cutter" shape and underestimates the true fraction within the cell

frac_mito <- mito_intden_corr/cell_intden_corr
 
corrected_mito_loc_xc <- cell_mito_xc_bkd %>% mutate(CellIntDenCorr = cell_intden_corr, MitoIntDenCorr = mito_intden_corr, FractionInMito = frac_mito)
...
# Background-corr. mean GFP in cell ------
cell_mean_corr <- corrected_mito_loc_xc$CellMean - corrected_mito_loc_xc$ExtracellularBackground

corrected_mito_loc_xc <- corrected_mito_loc_xc %>% 
  mutate(CellMeanCorr = cell_mean_corr)
